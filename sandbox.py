...
firemaking_xp_search_mask = cv.imread('C:\\Users\\Jeff C\\Downloads\\Code\\OpenCV files\\Varrock Firemaker\\image library\\firemaking_xp_search_mask.png',cv.IMREAD_GRAYSCALE)
firemaking_xp_vision = Vision('C:\\Users\\Jeff C\\Downloads\\Code\\OpenCV files\\Varrock Firemaker\\image library\\firemaking_xp.png', method = cv.TM_CCOEFF_NORMED, imread = cv.IMREAD_GRAYSCALE, search_mask = firemaking_xp_search_mask)
...
BLINDNESS_BORDER = 230 # 185 is ok for a full screen on y axis on my samsung monitor. I'm setting it to zero for testing purposes.
...
def restock():
    ...
    bank_click_point = bank_action.click(bank_screen_point, speed = speed(), wait=wait(), tick_dropper_odds= 100)
...
def burn_cycle():
    ...
    if teleport_location == 'falador':
        ...
        #click falador tele
        falador_tele_screen_point = wincap.get_screen_position(falador_tele_best_point)
        falador_tele_click_point = falador_tele_action.click(falador_tele_screen_point, speed = (speed()-.2), wait=(wait()+.2), tick_dropper_odds= 100)
        ...
        lumbridge_tele_click_point = lumbridge_tele_action.click(lumbridge_tele_screen_point, speed = (speed()-.2), wait=(wait()+.2), tick_dropper_odds= 100)
        time.sleep(.11 + abs(np.random.normal(0,.1)))
        pyautogui.click()
        ...
        varrock_tele_click_point = varrock_tele_action.click(varrock_tele_screen_point, speed = (speed()-.2), wait=(wait()+.2), tick_dropper_odds= 100)
        time.sleep(.11 + abs(np.random.normal(0,.1)))
        pyautogui.click()
    ...
    while len(inv_log_all_points) > 1:
        ...
        print('looking for firemaking_xp below blindness border')
        start_time = time.time()
        while True:
            screenshot = wincap.get_screenshot()
            screenshot = cv.cvtColor(screenshot, cv.COLOR_BGR2GRAY)        
            ...
            if firemaking_xp_confidence > FIREMAKING_XP_THRESHOLD and firemaking_xp_best_point[1] > BLINDNESS_BORDER:
                #time.sleep(.15 + abs(np.random.normal(0,.08)))
                tick_dropper()
                pyautogui.click()
                print('firemaking_xp spotted at %s | confidence %s | search time %s | Y Coord %s | Clicking log and ending firemaking_xp search loop' % (firemaking_xp_best_point, firemaking_xp_confidence, time.time() - start_time, firemaking_xp_best_point[1]))
                break
            
            #this just debugs what's going on in cases were you haven't yet found a suitable firemaking_xp drop
            print('firemaking_xp num %s | confidence %s | search_time %s | Y coord %s' % (len(firemaking_xp_all_points), firemaking_xp_confidence, time.time() - start_time, firemaking_xp_best_point[1] ))
